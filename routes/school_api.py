...
@router.get("/get-student/{student_id}", response_model=Student, dependencies=[Depends(jwtBearer())])
@log_request
@log_success
@log_error
...
@router.post("/add-student", dependencies=[Depends(is_admin)], response_model=Student)
@log_request
@log_success
@log_error
...
def get_students_in_class(class_name: str = Path(..., description="The class that you would view its students")):
    ...
    class_students = [stu["name"] for stu in students if class_name in stu["classes"]]
    if class_students:
